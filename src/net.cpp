...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        {
            addrConnect = CService("108.166.181.186", 80); // twehost getip.twecoin.org

            if (nLookup == 1)
            {
                CService addrIP("getip.twecoin.org", 80, true);
                if (addrIP.IsValid())
                    addrConnect = addrIP;
            }

            pszGet = "GET /simple/ HTTP/1.1\r\n"
                     "Host: getip.twecoin.org\r\n"
                     "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                     "Connection: close\r\n"
                     "\r\n";
...
static const char *strMainNetDNSSeed[][2] = {
	{"twecoin.org", "seed.twecoin.org"},
	{"twecoin.com", "seed.twecoin.com"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Twecoin is probably already running."), addrBind.ToString().c_str());
        else
