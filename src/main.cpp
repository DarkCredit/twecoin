...
uint256 hashGenesisBlock("0x000000143eca42afd45d792ede111788fb2005394e43cb2f9a05365d0cfa2782");
static const unsigned int timeGenesisBlock = 1396380526;
...
const string strMessageMagic = "Twecoin Signed Message:\n";
...
static const int64 nTargetTimespan = 8 * 60 * 60; // 8h
static const int64 nTargetSpacing = 2 * 60; // 120s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int64 LimitUp = nTargetTimespan * 100 / 200;
    int64 LimitUp2 = nTargetTimespan * 100 / 115;

    if (nActualTimespan < nTargetTimespan/4 && pHeight >= 24001)
       {
        nActualTimespan = LimitUp2;
       }
    if (nActualTimespan < LimitUp && pHeight <= 24000)
       {
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000005ba4e976bd17ccd0c3f054f1754e09cd14643b3537b4575c6b9fee922");
    }
...
*/
        ...
        // Genesis block
        const char* pszTimestamp = "Apple in talks to buy Japanese chipmaker";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = 329053763;

        if (fTestNet)
        {
            block.nTime    = 1396534177;
            block.nNonce   = 133773564;
        }
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0x1effccd632ae20c9daae30c3488f4470f8d32607b47664c767f7e87eb06c4bc4"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xbe, 0xb6, 0xdb };
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = Hashtwe(BEGIN(hashRand), END(hashRand));
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        hashKey = Hashtwe(BEGIN(hashKey), END(hashKey));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        uint256 hash = Hashtwe(vRecv.begin(), vRecv.begin() + nMessageSize);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TwecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TwecoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("TwecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("twecoin-miner");
    ...
    try { loop {
        ...
        printf("Running TwecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("TwecoinMiner terminated\n");
        throw;
    }
}
